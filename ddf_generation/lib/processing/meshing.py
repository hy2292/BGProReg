...
#Important functions
def get_FEM_displacements(mesh, surface_node_idx, surface_displacements, time_steps=4, material='neo-Hookean', **material_properties):
    '''
...
def get_surface_mesh(mesh):
    '''
        ...
        original point indices, original point positions, and the surface mesh  返回原始点索引 原始点位置 表面网格
    '''
...
class PlotCallback():
    ...
    def __init__(self, points_moving, points_fixed=None, to_numpy= lambda a:a, hide_axis= False, size=[8, 8],
                 plot_interval=1,
                 axis=[0,2,1], # 原始 0 2 1
                 mode='3d', moving_color='r', fixed_color='b', save_as=None,
                 moving_last_N=0, fixed_last_N=0, invert_yaxis=True, plot_show=True,
    ...
    def _plot(self, tf_param):
        ...
        #save = f"/home/dasong/BIGM-motion-tracking-main/data/new-MRI-US/cpd_results/{self.save_as}_{self.count}.png"
        #if self.save_as is not None: plt.savefig(save, dpi=300)
