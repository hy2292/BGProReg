...
class FEBioModel():
    ...
    def add_material(self, type, object_name, name=None, **properties):
        ...
        #         # 如果属性值是字典，创建子标签，并添加其内部键值对
        #         sub_tag = ET.SubElement(material, prop_name,
        #                                 type=prop_value.get('type', ''))  # 如 <elastic type="neo-Hookean">
        #         # 处理子标签内的键值对（如 E、v）
        #         for sub_key, sub_value in prop_value.items():
        #             if sub_key != 'type':  # 'type' 已作为属性添加，避免重复
        #                 ET.SubElement(sub_tag, sub_key).text = str(sub_value)
    ...
    def run(self, **kwargs):
        ...
        #Simulate
        command= subprocess.run(['/home/dasong/FEBioStudio/bin/febio4', '-i', f'{self.name}.feb'])
